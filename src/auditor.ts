import { GitHubIssue, DataQualityReport, FlaggedIssue } from "./types";
...
export function auditDataQuality(issues: GitHubIssue[]): DataQualityReport {
  const flagMap = new Map<number, FlaggedIssue>();

  function addFlag(issue: GitHubIssue, flag: string) {
  ...
  // ── Duplicate titles ──────────────────────────────────────────────────
  const titleCounts = new Map<string, GitHubIssue[]>();
  for (const issue of issues) {
